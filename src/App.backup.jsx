...
function GoalModal({ onSelect, onClose }) {
  ...
  const handleGoalSelect = async (goal) => {
    ...
    try {
      ...
      onSelect({ goal: goal, aiText: text }); // Immediately pass AI data and close
    } catch (err) {
...
export default function App() {
  ...
  const [authLoading, setAuthLoading] = useState(true); // loading state for auto-login check
  ...
  const handleLogout = async () => {
    await apiLogout(); // calls POST /api/logout and clears localStorage
    setLoggedIn(false);
    setUserInfo(null);
    setPage('dashboard');
  };
